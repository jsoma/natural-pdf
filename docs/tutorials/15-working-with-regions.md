...
The `.above()` method creates a region above an element, perfect for capturing headers:
...
# Find a landmark and get everything above it
header_text = page.find('text:contains("Violations")')
region = header_text.above()
region.show()

# Get above with limited height
content_region = region.above(height=50)
content_region.extract_text()
```

## Combining Regions with + Operator

You can combine multiple regions using the `+` operator:
...
# Add combined regions as exclusions
page.add_exclusion(top)
page.add_exclusion(bottom)
```

## Using expand() for Flexible Boundaries

The `.expand()` method creates larger regions from elements:
...
When finding elements in regions, use `overlap='partial'` to include elements that extend beyond boundaries:
