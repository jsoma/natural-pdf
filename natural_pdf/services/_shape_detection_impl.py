"""Shape detection implementation (lines, blobs) for the shapes service.

This module holds the implementation that previously lived on
``natural_pdf.analyzers.shape_detection_mixin.ShapeDetectionMixin``.  Functions
take the host object (Page, Region, or any object resolvable by
``resolve_page_context``) as their first argument.  ``ShapeDetectionService``
delegates here; the old mixin module remains as a thin compatibility stub.
"""
...
def get_image_for_detection(
    ...
    try:
        page_obj_result = resolve_page_context(host)
    ...
    # For a Page, host.width/height are PDF points. pil_image.width/height are pixels.
    # For a Region, host.width/height are PDF points of the region. pil_image.width/height are pixels of the cropped image.
...
def convert_line_to_element_data(
    line_data_img: Dict,
    scale_factor: float,
    ...
    if scale_factor == 0:
        ...
        # This situation ideally shouldn't happen if get_image_for_detection is robust.
        effective_scale = 1.0
    else:
...
def detect_lines(
    ...
    Args:
        host: The Page/Region/collection to detect lines on.
    ...
    if method == "projection":
        return detect_lines_projection(
            host,
            resolution=resolution,
            source_label=source_label,
            horizontal=horizontal,
...
def detect_line_element_data(
    ...
    cv_image, scale_factor, origin_offset_pdf, page_object_ctx = get_image_for_detection(
        host, resolution
    )
    if cv_image is None or page_object_ctx is None:
        logger.warning(f"Skipping line detection for {host} due to image error.")
        return []
    ...
    if method == "projection":
        ...
        lines_data_img, _profile_h_smoothed, _profile_v_smoothed = find_lines_on_image_data(
            cv_image=cv_image,
        ...
        lines_data_img = process_image_for_lines_lsd(
            cv_image,
            off_angle,
            min_line_length,
        ...
        lines_data_img = [
            ...
            if (
                horizontal
                and abs(line_data["y2"] - line_data["y1"]) < abs(line_data["x2"] - line_data["x1"])
            )
            or (
                vertical
                and abs(line_data["y2"] - line_data["y1"]) >= abs(line_data["x2"] - line_data["x1"])
...
def detect_lines_projection(
    ...
    """Internal function for projection profiling line detection."""
    cv_image, scale_factor, origin_offset_pdf, page_object_ctx = get_image_for_detection(
        host, resolution
    )
    if cv_image is None or page_object_ctx is None:
    ...
    # get_image_for_detection() already rendered the exact image being processed.
    # find_lines_on_image_data() only needs image dimensions, so avoid a
    # second render here.
    pil_image_for_dims = Image.fromarray(cv_image)
    ...
    lines_data_img, profile_h_smoothed, profile_v_smoothed = find_lines_on_image_data(
        cv_image=cv_image,
        pil_image_rgb=pil_image_for_dims,
        horizontal=horizontal,
        vertical=vertical,
    ...
    for line_data_item_img in lines_data_img:
        element_constructor_data = convert_line_to_element_data(
            line_data_item_img, scale_factor, origin_offset_pdf, page_object_ctx, source_label
        )
...
def detect_lines_lsd(
    ...
    """Internal function for LSD line detection."""
    try:
        import cv2
    except ImportError:
        raise ImportError(
    ...
    if cv_image is None or page_object_ctx is None:
        logger.warning(f"Skipping LSD line detection for {host} due to image error.")
        return host
...
def process_image_for_lines_lsd(
    cv_image: np.ndarray,
    off_angle: int,
    min_line_length: int,
    merge_angle_tolerance: int,
    ...
    def get_line_properties(line_data_item):
        ...
        is_h = (
            abs(normalized_angle_deg) <= off_angle or abs(normalized_angle_deg - 180) <= off_angle
        )
...
def detect_blobs(
    ...
    Args:
        host: The Page/Region to detect blobs on.
    ...
    cv_image, scale_factor, origin_offset_pdf, page_obj = get_image_for_detection(host, resolution)
