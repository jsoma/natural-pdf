...
"""Interactive HTML/JS element viewer for Jupyter notebooks.

The viewer renders a page image plus positioned element overlays and displays
them via IPython. It is a plain Python object, not an ipywidgets DOMWidget —
all interactivity (zoom, pan, element info) lives in injected JavaScript.
ipywidgets/IPython are hard dependencies of natural-pdf, so no availability
...
# Attributes exposed in the info panel by default.
...
class InteractiveViewerWidget:
    def __init__(self, pdf_data: Optional[Dict[str, Any]] = None, **kwargs):
        """
        ...
        if pdf_data:
            ...
            self.pdf_data = {
                "page_image": kwargs.get("image_uri", ""),
                "elements": kwargs.get("elements", []),
            }

        logger.debug(
            "InteractiveViewerWidget initialized with %d elements",
            len(self.pdf_data.get("elements", [])),
        )
    ...
    def _add_javascript(self):
        ...
        # The JS payload only needs the element data; the page image is
        # already embedded in the <img> tag and would double the notebook
        # size if serialized again here.
        js_payload = {"elements": self.pdf_data.get("elements", [])}
    ...
    def from_page(
        ...
        *,
        resolution: int = 150,
        elements_to_render: Optional[List[Any]] = None,
        ...
        Args:
            ...
            elements_to_render: Explicit list of elements to overlay. Defaults
                to every element on the page (exclusions NOT applied — this is
                a debugging view, so nothing is hidden).
        ...
        Raises:
            Exception: Propagates rendering/data-preparation failures.
        """
        ...
        if elements_to_render is None:
            # A debugging viewer must show everything, including elements that
            # fall inside exclusion zones.
            page_elements = list(page.find_all("*", apply_exclusions=False))
        else:
            page_elements = list(elements_to_render)
        ...
        elements = []
        all_attrs_to_check = _DEFAULT_ELEMENT_ATTRIBUTES + (include_attributes or [])
