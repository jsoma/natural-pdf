...
# =============================================================================
# Redirect the OCR result cache to an isolated temp directory so that
# (a) tests don't pollute the real user cache, and (b) mocked-OCR tests
# can't be poisoned by other tests in the same session.  Function-scoped
# so each test starts with an empty cache.
@pytest.fixture(autouse=True)
def _isolate_ocr_cache(tmp_path):
    from natural_pdf.ocr.ocr_cache import OCRCache, set_default_cache

    previous = set_default_cache(OCRCache(cache_dir=tmp_path / "ocr_cache"))
    yield
