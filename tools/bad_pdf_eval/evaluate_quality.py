...
def analyze_code_quality(code: str) -> Dict[str, Any]:
    ...
    quality = {
        "uses_guides": bool(re.search(r'from natural_pdf\.analyzers import Guides|Guides\(|Guides\.', code)),
        "uses_tatr": bool(re.search(r'analyze_layout\([\'"]tatr[\'"]?\)', code)),
        "uses_extract_table": bool(re.search(r'\.extract_table\(\)', code)),
        "has_placeholder_text": bool(re.search(r'[\'"](?:AnchorText|Texts|Also|HeaderText|TableHeader)[\'"]', code)),
        "uses_real_text": bool(re.search(r'[\'"](?!AnchorText|Texts|Also|HeaderText|TableHeader)[^\'"{}\[\]]+[\'"]', code)) and not bool(re.search(r'[\'"](?:AnchorText|Texts|Also|HeaderText|TableHeader)[\'"]', code)),
...
def evaluate_submission(submission_path: Path) -> Dict[str, Any]:
    ...
    if data.get("difficult_elements"):
        result["difficult_elements_analysis"] = analyze_difficult_elements(data["difficult_elements"])
...
def main():
    ...
    for result in results:
        if result["doc_code_quality"]:
            ...
            if quality["uses_guides"]: stats["using_guides"] += 1
            if quality["uses_tatr"]: stats["using_tatr"] += 1
            if quality["has_placeholder_text"]: stats["using_placeholders"] += 1
    ...
    print(f"  Using Guides API: {stats['using_guides']} ({stats['using_guides']/stats['with_doc_enrichment']*100:.1f}%)")
    print(f"  Using TATR: {stats['using_tatr']} ({stats['using_tatr']/stats['with_doc_enrichment']*100:.1f}%)")
    print(f"  Using placeholders: {stats['using_placeholders']} ({stats['using_placeholders']/stats['with_doc_enrichment']*100:.1f}%)")
    print(f"  Average quality score: {stats['avg_quality_score']:.1f}/12")
    
    print(f"\nDifficult Elements Identified:")
    for elem_type, count in sorted(stats["difficult_elements_breakdown"].items(), key=lambda x: x[1], reverse=True):
