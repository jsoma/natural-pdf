...
from tools.bad_pdf_eval.llm_enrich import (
    read_md, img_to_b64_jpeg, build_prompt, build_pdf_prompt,
    DocOutput, EVAL_DIR, CHEATSHEET_PATH, WORKFLOWS_PATH, 
    DECISION_TREE_PATH
)
...
class RetryOutput(BaseModel):
    ...
    thought_process: str = Field(..., description="Revised reasoning addressing the specific feedback")
    code_suggestion: str = Field(..., description="Improved Python snippet addressing all feedback points")


def build_retry_prompt(original_code: str, quality_analysis: Dict[str, Any], 
...
"""
    ...
    messages = [
        {"role": "system", "content": "You are a Natural PDF expert. Improve the code based on specific feedback."},
...
def enrich_with_retry(summary_path: Path, api_key: str, model: str = "gpt-4o", 
                     retry_threshold: int = 6, max_retries: int = 2):
    ...
    if not FORCE and summary.get("thought_process") and summary.get("code_suggestion") and summary.get("attempts"):
    ...
    summary["difficult_elements"] = getattr(best_doc, "difficult_elements", summary.get("difficult_elements"))
    ...
    print(f"[update] Best score: {best_score}/12 after {retry_count} retries (kept all {len(attempts)} attempts)")
...
def main():
    ...
    ap.add_argument("--retry-threshold", type=int, default=6, 
                    help="Minimum quality score before retry")
    ...
    if args.workers <= 1:
        ...
        for p in paths:
            try:
                enrich_with_retry(p, args.api_key, args.model,
                                  args.retry_threshold, args.max_retries)
